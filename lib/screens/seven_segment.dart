...
class _ClockPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < 4; i++) {
      _paintDigit(canvas, Offset(xs[i] + unit * 0.25, 0), digitW, h, digits[i], glow);
    }
    _paintColon(canvas, Offset(unit * 6.15, 0), unit * 0.7, h);
  }

  void _paintDigit(Canvas c, Offset o, double w, double h, int d, Paint glow) {
    final lit = _digitSegments[d] ?? '';
    final t = w * 0.16; // segment thickness
    final litPaint = Paint()..color = color;
    final ghostPaint = Paint()..color = LedColors.ghost;

    Path hSeg(double y) => _hex(o.dx + t * 0.6, o.dy + y, w - t * 1.2, t, horizontal: true);
    Path vSeg(double x, double y) => _hex(o.dx + x, o.dy + y, t, h / 2 - t, horizontal: false);
  ...
  Path _hex(double x, double y, double w, double h, {required bool horizontal}) {
    final p = Path();
    if (horizontal) {
      final k = h / 2;
      p
